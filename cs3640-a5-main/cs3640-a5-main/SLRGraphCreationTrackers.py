...
df = pd.read_csv('DataTrackerResults.csv')


# Correct column names
trackers_column = 'Number of Trackers'
loading_time_column = 'With Tracker Load Time'

# Plot the graph with correct data
plt.figure(figsize=(10, 6))
sns.regplot(x=trackers_column, y=loading_time_column, data=df, scatter_kws={'s':50})
plt.title('Number of Trackers vs. Loading Time when not blocking trackers')
...
plt.savefig('Graphs/WithTrackers.png')
plt.show()
